module quadrature_mod
  ...
  type, public :: quadrature_t
     ...
     real (kind=longdouble_kind), dimension(:,:), pointer :: leg
     real (kind=longdouble_kind), dimension(:,:), pointer :: der
  end type quadrature_t

  public  :: gausslobatto
  public  :: gausslobatto_leg
  public  :: test_gausslobatto
  public  :: gauss
  public  :: test_gauss
  public  :: legendre
  public  :: legendre_deriv
  public  :: jacobi
  public  :: quad_norm
  public  :: get_gll
...
contains
  ...
  subroutine gausslobatto_leg(nx, nt,gll)
    ...
    allocate(gll%leg(nx, nt+1))
    allocate(gll%der(nx, nt+1))
    
    do i = 1, nx
	call legendre_deriv(gll%points(i),nt,gll%leg(i,:),gll%der(i,:))
    end do 

  end subroutine gausslobatto_leg
  ...
  subroutine test_gausslobatto(npts,nct, gll)
    use kinds, only : real_kind
    integer, intent(in) :: npts, nct
    type (quadrature_t) :: gll
...
!	author : hy
!	legendre derivative 
subroutine  legendre_deriv(x,N, leg, der)

    integer   :: N
    real (kind=longdouble_kind) :: x
    real (kind=longdouble_kind) :: leg(N+1)
    real (kind=longdouble_kind) :: der(N+1)

    real (kind=longdouble_kind) ::  p_1, p_2, p_3
    real (kind=longdouble_kind) ::  d_1, d_2, d_3
    integer   :: k
    p_3 = 1.0_longdouble_kind
    d_3 = 0.0_longdouble_kind
    leg(1)=p_3
    der(1)=0.0
    if (n.ne.0) then
       p_2 = p_3
  ...
  subroutine get_gll(npt, nct, gll)
    integer, intent(in) :: npt, nct
    type(quadrature_t):: gll 

	write(*,*) "Gausslobatto gll "
	gll = gausslobatto(npt)
	call gausslobatto_leg(npt, nct,gll)
	call test_gausslobatto(npt,nct, gll)
